...
public class trade_list : MonoBehaviour
{
    ...
    public List<trade> list = new List<trade>();
    ...
    public void Initialize(int number)
    {
        ...
        {
            list.Add(Instantiate(trade, this.transform).GetComponent<trade>());//instancie le trade et l'ajoute a la liste
        }
    ...
    public void Detail(double id_trade, trade trade_panel)
    {
        ...
        dtl_prix_dachat.text = "Prix D'achat : " + trade_panel.data.prix_achat.ToString() ;
        ...
        {
            roi = trade_panel.prix * trade_panel.data.quantite - trade_panel.data.prix_achat * trade_panel.data.quantite * (1f - 1f / trade_panel.leverage);
            profit_dollard = roi - trade_panel.data.prix_achat* trade_panel.data.quantite / trade_panel.leverage;
        }else if (trade_panel.type_trade == "Short")
        {
            roi = trade_panel.data.prix_achat * trade_panel.data.quantite - trade_panel.prix * trade_panel.data.quantite * (1f - 1f / trade_panel.leverage);
            profit_dollard = roi - trade_panel.prix * trade_panel.data.quantite / trade_panel.leverage;
        }
        ...
        GameManager.instance.concatenate_money(trade_panel.data.quantite, ref temp_float, ref temp_scale);
        dtl_quantite_crypto.text = temp_float.ToString()+ temp_scale + " " + UIManager.contrat.sigle;
        GameManager.instance.concatenate_money((trade_panel.data.quantite * trade_panel.prix), ref temp_float, ref temp_scale);
        dtl_quantite_dollard.text = temp_float.ToString() + temp_scale + " $";
        dtl_paire.text = UIManager.contrat.sigle;
        dtl_type.text = trade_panel.type_trade;
        idtrade = trade_panel.data.id;
        quantite = trade_panel.data.quantite;
        GameManager.instance.concatenate_money(trade_panel.data.liquidation, ref temp_float, ref temp_scale);
    ...
    IEnumerator ClotureTrade(string quantite)
    {
        ...
        UnityWebRequest www = UnityWebRequest.Post("http://localhost:5000/cloture", form);
